...
(* 请在此处解答 *) Proof. simpl. reflexivity.  Qed.
...
Example S_plus_eqb : forall n : nat,
  (S n + 1) =? S (n + 1) = true.
Proof.
...
Example S_plus_eqb_ : forall n : nat,
  (S n + 1) =S (n + 1) .
Proof.
  intros n.
  reflexivity. 
...
Proof.
  ...
  intros [] [].
  - intros. reflexivity.
  - intros. replace true with (true || false) by reflexivity.
    replace false with (true && false) at 2 by reflexivity.
    rewrite -> H. reflexivity.
  - intros. replace true with (false || true) by reflexivity. 
      replace false with (false && true) at 1 by reflexivity.
      rewrite -> H. reflexivity.
  - intros. reflexivity.
Qed. 
...
Fixpoint bin_to_nat (m:bin) : nat
  ...
  match m with
  | Z => 0
  | A m' => (S (S 0)) * (bin_to_nat m')
  | B m' => (S (S 0)) * (bin_to_nat m') + 1
  end.
