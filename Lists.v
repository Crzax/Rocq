...
Fixpoint remove_one (v:nat) (s:bag) : bag
  ...
  match s with
  | nil => nil
  | v' :: s => if v =? v' then s else v' :: remove_one v s
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag
  ...
  match s with
  | nil => nil
  | v' :: s => if v =? v' then remove_all v s else v' :: remove_all v s
  end.
...
Fixpoint subset (s1:bag) (s2:bag) : bool
  ...
  match s1 with
  | nil => true
  | v :: s1 => if count v s2 =? 0 then false else subset s1 (remove_one v s2)
  end.
...
Qed.
*)
Lemma Identity : forall (a:nat), a =? a = true.
Proof.
  intros a. induction a as [| n IHn].
  - (* a = 0 *)
    reflexivity.
  - (* a = S n *)
...
Proof.
  intros v s. induction s as [| n s' IHs'].
  - (* s = nil *)
    simpl. rewrite -> Identity. reflexivity.
  - (* s = cons n s' *) 
    simpl. rewrite <- IHs'. rewrite -> Identity. reflexivity.
Qed.
...
Proof.
  (* 请在此处解答 *) 
 intros l. induction l as [| n l' IHn'].
  - (* l = nil *)
    reflexivity.
  - (* l = cons *)
...
Proof.
  ...
  intros l. induction l as [| n l' IHl'].
  - reflexivity.
  - simpl. rewrite -> Identity. rewrite <- IHl'. reflexivity. Qed. 
(** [] *)
...
(* 请在此处解答
    ...
    [] *)
Theorem bag_count_sum : forall (a : bag) (b : bag),
  count 0 (sum a b) = count 0 a + count 0 b.
  Proof.
  intros a b. induction a as [| n a' IHa'].
  - reflexivity.
  - destruct n.
...
Proof.
  (* 请在此处解答 *) 
  intros x. destruct x. simpl. rewrite Identity. reflexivity.
Qed.
(** [] *)
...
(* 请在此处解答 *)
(* 答案：0 个。因为 baz 的两个构造子 Baz1 和 Baz2 都需要
   一个已有的 baz 类型的值作为参数，不存在不需要 baz 参数的
   基础构造子，因此无法构造出任何 baz 类型的值。 *)
